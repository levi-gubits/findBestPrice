    ...
    namespace Agent{
        ...
        class Search implements Isearch{
           ...
           function __construct($argv){
                ...
                $url = "https://www.cameraplus.co.il/search/?q=".$this->product;
                $response = file_get_contents($url);

                // find link
                $_title = '<div class="element_items_list_items ">';
                $response = substr($response, strpos($response, $_title) + strlen($_title), strlen($response));
                $this->link = trim(substr($response, 0, strpos($response,'<img')));

                $_title = '<a class="price" href="  /items/';
                $this->link = substr($this->link, strpos($this->link, $_title) + strlen($_title), strlen($this->link));
                $this->link = trim(substr($this->link, 0, strpos($this->link,'"')));
                $this->link = "https://www.cameraplus.co.il/items/".$this->link;
           ...
           public function SendInfoToModule (){
            ...
            if($this->price != 0){
                $result = Database::command("INSERT INTO `products_stores`(`proid`, `price`, `stor_id`,`link`) VALUES ($this->proid,$this->price,15,'$this->link')");
            }else{
